...
class Onboarding extends Component {
	...
	render() {
		...
		const options = roles.map((role, i)=>{
			return <option key={i+role} value={role}>{role}</option>						
		});
		return (
				...
				{isMobile?<div style={{width: '100%'}}>
					<div className='mobileFilters'>				
						...
						<select onChange={(e)=>roleFilter(e.target.value)} className='mobileSelect' value={role}>
							<option key={"0all"} value={'all'}>Show All ({original.length})</option>
							{options}
						</select>
					</div>
				...
				<h3 className="filterHeader">Show</h3>
					<div onClick={roleFilter.bind(null, 'all')} className={role==='all'?"filterMain":"filter"}>All Applicants ({original.length})</div>
					{roles.map((roley,ind)=> <div key={ind} onClick={roleFilter.bind(null, roley)} className={role===roley?"filterMain":"filter"}>{roley}</div>)}
				</div>
				<div className="table">
